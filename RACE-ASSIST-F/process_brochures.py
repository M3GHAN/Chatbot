...
class BrochureProcessor:
    def __init__(self, brochure_folder: str = "brochures", output_folder: str = "faiss_databases"):
        ...
        self.mapping_file = "unified_mapping.json"
        self.unified_index_path = os.path.join(output_folder, "complete_unified_index.faiss")
        self.unified_metadata_path = os.path.join(output_folder, "complete_unified_metadata.pkl")
    ...
    def load_unified_index(self) -> Tuple[faiss.Index, List[str], List[Dict]]:
        """Load unified FAISS index and metadata"""
        # Check if unified database files exist
        if not os.path.exists(self.unified_index_path):
            raise FileNotFoundError(f"Unified index file not found: {self.unified_index_path}")
        if not os.path.exists(self.unified_metadata_path):
            raise FileNotFoundError(f"Unified metadata file not found: {self.unified_metadata_path}")
        
        # Load FAISS index
        index = faiss.read_index(self.unified_index_path)
        
        # Load metadata
        with open(self.unified_metadata_path, 'rb') as f:
    ...
    def search_similar(self, query: str, top_k: int = 5) -> List[Tuple[str, float, str]]:
        ...
        for i, (score, idx) in enumerate(zip(scores[0], indices[0])):
            if idx < len(chunks):
                source_file = chunk_metadata[idx].get('source_file', 'Unknown') if idx < len(chunk_metadata) else 'Unknown'
                results.append((chunks[idx], float(score), source_file))
        
        return results
    
    def get_available_files(self) -> List[str]:
        """Get list of files available in the unified database"""
        try:
            if os.path.exists(self.mapping_file):
                with open(self.mapping_file, 'r') as f:
                    mapping = json.load(f)
                if 'unified_database' in mapping:
                    return mapping['unified_database'].get('files_processed', [])
            return []
        except Exception as e:
            print(f"Error loading available files: {str(e)}")
            return []
